...
const schema = yup.object().shape({
  ...
  phone: yup.string().required("Phone number is required").matches(/^\d{10}$/, "Phone must be 10 digits"),
...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (res.data.status) {
        Swal.fire({ title: "Registration Successful 🎉", text: `Welcome, ${data.name}!`, icon: "success" });
        navigate("/userlogin");
        return;
      }
      Swal.fire({ title: "Registration Failed ❌", text: res.data.message || "Please try again", icon: "error" });
    } catch (err) {
      Swal.fire({ title: "Server Error", text: err.response?.data?.message || err.message, icon: "error" });
    }
  };
    ...
    <section>
      ...
      <Container>
        <Row className="justify-content-center" style={{ marginTop: "10px" ,height: "650px"}}>
          <Col xl={10}>
            <Card className="border-0 shadow cardd" style={{ width: "800px", height: "650px", marginLeft: "100px" ,marginTop:"100px"}}>
              <Card.Body className="p-0">
                <Row>
                    ...
                    <div className="p-4">
                      ...
                      <Form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <Form.Group className="mb-2">
                          ...
                          <Form.Control as="textarea" placeholder="Enter your address" rows={2} {...formRegister("address")} />
                  ...
                  <Col lg={6} className="d-none d-lg-block">
                    <div className="h-100 d-flex flex-column justify-content-center align-items-center text-white p-4 rounded-end"
